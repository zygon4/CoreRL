...
import com.zygon.rl.util.quest.Graph.Edge;
import com.zygon.rl.util.quest.Graph.Vertex;

// From https://github.com/phishman3579/java-algorithms-implementation
class AStar<T extends Comparable<T>> {

    public AStar() {
    }

    /**
     * Find the path using the A* algorithm from start vertex to end vertex or
     ...
     */
    public List<Graph.Edge<T>> aStar(Graph<T> graph, Graph.Vertex<T> start,
            Graph.Vertex<T> goal) {
    ...
    /**
     * Default distance is the edge cost.If there is no edge between the start
     * and next then it returns Integer.MAX_VALUE;
     *
     * @param start
     * @param next
     * @return
    ...
    protected int heuristicCostEstimate(Graph.Vertex<T> start,
            Graph.Vertex<T> goal) {
        return 1;
    }
